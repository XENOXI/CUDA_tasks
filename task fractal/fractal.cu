...
int julia(int x, int y)
{
    ...
    // Проверяем быстро ли растет значение |a|*|a|, где a = a*a + c в точке (x,y).
    // В данном случае, проверяется превзойдет ли |a|*|a| значения в одну тысячу, спустя двести итерации.
